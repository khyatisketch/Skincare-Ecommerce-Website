...
export default function AdminReviews() {
  ...
  const fetchReviews = async () => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/products/admin/reviews`, {
  ...
  const approveReview = async (id) => {
    try {
      await axios.patch(
        `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/products/admin/reviews/${id}/approve`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      )
      setReviews((prev) =>
  ...
  const deleteReview = async (id) => {
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/products/admin/reviews/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
