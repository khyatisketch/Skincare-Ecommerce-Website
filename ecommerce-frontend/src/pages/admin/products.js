...
//   const router = useRouter();
  ...
  const fetchProducts = async (token) => {
    try {
      const res = await axios.get('http://localhost:4007/products/getAllProducts', {
  ...
  const fetchCategories = async (token) => {
    try {
      const res = await axios.get('http://localhost:4007/categories/getAllCategories', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleCreateOrUpdate = async (e) => {
    ...
    try {
      ...
      const url = editingProduct
        ? `http://localhost:4007/products/updateProduct/${editingProduct.id}`
        : 'http://localhost:4007/products/createProduct';
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:4007/products/deleteProduct/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchProducts(token);
    } catch (err) {
...
{p.stock > 0 && p.stock < 5 && (
  ...
  <button
    onClick={async () => {
      try {
        await axios.patch(`http://localhost:4007/products/admin/products/${p.id}/restock`, {
